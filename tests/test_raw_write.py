...
def test_write_raw_dtype_preserved(_make_dataset):
    ...
    shape = (5, 8, 8)
    dataset, _ = _make_dataset(shape, dtype=np.float32)
...
def test_write_raw_z_chunks(_make_dataset):
    ...
    shape = (30, 20, 10)
    # 3 chunks along Z
    dataset, _ = _make_dataset(shape, chunks=(10, 20, 10))
...
def test_write_raw_arbitrary_chunks(_make_dataset):
    ...
    dataset, _ = _make_dataset(shape, chunks=(8, 10, 4))
...
def test_write_via_dataset_to_path(_make_dataset):
    ...
    shape = (8, 12, 16)
    dataset, _ = _make_dataset(shape, chunks=(4, 6, 4))
